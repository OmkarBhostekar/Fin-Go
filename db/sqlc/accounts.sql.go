...
func (q *Queries) GetAcountById(ctx context.Context, id int64) (Account, error) {
	row := q.db.QueryRowContext(ctx, getAcountById, id)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
...
SELECT id, owner, balance, currency, "createdAt" FROM accounts WHERE id = $1 LIMIT 1 FOR NO KEY UPDATE
`
